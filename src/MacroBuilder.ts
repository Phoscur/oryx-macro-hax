    ...
    const self: MacroBuilder = {
        ...
        tapKey: (rawKey: string, msDelayAfter: number) => {
            if (!rawKey.startsWith("X_")) {
                throw new Error(
                    "rawKey must come from the underlying language thingy, maybe you meant X_" +
                        rawKey.toUpperCase() +
                        " or something?",
                );
            }
        ...
        typeAlphanumeric: (
            ...
            for (let i = 0; i < strToType.length; i++) {
                ...
                if (char !== char.toLowerCase()) {
                    self.withShift(
                        newMacro().typeAlphanumeric(char.toLowerCase()),
                    );
                } else if (
                    ...
                    throw Error(
                        "Unsupported char in typeAlphanumeric: " +
                            char +
                            ", use typeRaw instead?",
                    );
                }
        ...
        build: () => {
            ...
            while (
                cmds.length > 0 &&
                cmds[cmds.length - 1].startsWith("SS_DELAY")
            ) {
